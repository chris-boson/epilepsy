...
\begin{enumerate}
  ...
  \item The output token sequence includes the specialized tokens (CLS), and (SEP), that not only help to mark the beginning and end of a sequece,
  but aid in downstream classification in the model.
  \item The word resection is split into two tokens, with a special marker for the continuation of a word that was split, namely ##
\end{itemize}
